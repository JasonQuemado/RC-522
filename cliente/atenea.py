...
class CourseManager(Gtk.Window):
    ...
    def loginScreen(self):
        ...
        contenedor = Gtk.Box(orientation=Gtk.Orientation.VERTICAL)
        contenedor.set_valign(Gtk.Align.CENTER)
        contenedor.set_halign(Gtk.Align.CENTER)
        ...
        contenedor.pack_start(self.botonLogin, False, False, 0)
        self.layout.pack_start(contenedor, True, True, 0)
    ...
    def tarjetaLeida(self, uid):
        try:
            with urlopen(f"{SERVER_URL}/students?student_id={uid}") as response:
    ...
    def queryScreen(self):
        ...
        #Nos dabaa error así que ponemos esto
        #self.result_grid = Gtk.Grid()
        ...
        #self.layout.pack_start(self.result_grid, True, True, 5)
    ...
    def consultaServidor(self, entry):
        ...
        ##if texto == "timetables":
        ##  now = datetime.now()
        ##  today = now.strftime("%a")       # Ej: 'Thu'
        ##  hour = now.strftime("%H:%M")     # Ej: '12:05'
        ##  texto = f"timetables?student_id={self.studentId}&today={today}&hour={hour}"
        if texto == "timetables":
            texto = f"timetables?student_id={self.studentId}"
        elif texto == "students":
            pass
        ...
        def peticion():
            try:
                with urlopen(f"{SERVER_URL}/{texto}") as response:
